...
def create_dataset_wav(input_wav, dataset, mode, audio_name):

    # Provide the path to the input WAV file
    ...
    os.makedirs(f"youtubeaudio", exist_ok=True)
    # copy file
    shutil.copy(input_wav, f"youtubeaudio/{AUDIO_NAME}.wav")
    ...
    return f"{AUDIO_NAME}.zip"

inputs = [
    gr.inputs.File(label="Input WAV"),
    gr.inputs.Radio(["Youtube", "Custom"], label="Dataset"), #, value="Custom"),
    gr.inputs.Radio(["Separate", "Splitting"], label="Mode"), #, value="Splitting"),
    gr.inputs.Textbox(label="Audio Name")
]

output = gr.outputs.File(label="Download ZIP")

gr.Interface(fn=create_dataset_wav, inputs=inputs, outputs=output).launch()
